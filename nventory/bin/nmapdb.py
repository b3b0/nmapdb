...
#
# Slightly modified (c) 2018 by Dustin Davis (b3b0) for https://github.com/b3b0/nventory
...
def main(argv, environ):
    ...
    for fname in args:
        ...
        for host in doc.getElementsByTagName("host"):
            ...
            myprint("[hosts] whois:\n")
            myprint("%s\n" % (whois_str))
            myprint("[hosts] Kernel_Admin:\t\t%s" % (Kernel_Admin))
            myprint("[hosts] Hardware_Admin:\t\t%s" % (Hardware_Admin))
            myprint("[hosts] Application_Admin:\t\t%s" % (Application_Admin))
            myprint("[hosts] Common_Name:\t\t%s" % (Common_Name))
            myprint("[hosts] Audit_Date:\t\t%s" % (Audit_Date))

            if nodb_flag == false:
                try:
                    cursor.execute("UPDATE hosts SET mac=?, hostname=?, protocol=?, os_name=?, os_family=?, os_accuracy=?, os_gen=?, last_update=?, state=?, mac_vendor=?, whois=? WHERE ip=?",
                            (mac, hostname, protocol, os_name, os_family, os_accuracy,
                            os_gen, timestamp, state, mac_vendor, whois_str, ip))
                    cursor.execute("INSERT or IGNORE INTO hosts (ip, mac, hostname, protocol, os_name, os_family, os_accuracy, os_gen, last_update, state, mac_vendor, whois) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            ...
            for port in ports:
                ...
                if nodb_flag == false:
                    try:
                        cursor.execute("INSERT or REPLACE INTO ports VALUES (?, ?, ?, ?, ?, ?, ?)", (ip, pn, protocol, port_name, state, service_str, timestamp))
                    except sqlite.IntegrityError, msg:
